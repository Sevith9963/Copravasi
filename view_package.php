...
if(isset($_GET['id'])){
    $id_hash = $_GET['id'];
    $stmt = $conn->prepare("SELECT * FROM `packages` WHERE md5(id) = ?");
    $stmt->bind_param("s", $id_hash);
    $stmt->execute();
    ...
    if($result->num_rows > 0){
        ...
        $stmt_review = $conn->prepare("SELECT r.*, CONCAT(firstname,' ',lastname) as name FROM `rate_review` r INNER JOIN users u ON r.user_id = u.id WHERE r.package_id = ? ORDER BY unix_timestamp(r.date_created) DESC");
        $stmt_review->bind_param("i", $id);
        $stmt_review->execute();
        $review = $stmt_review->get_result();
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-7">
                ...
                <hr class="border-warning">
                <small class='text-muted'>Location: <?php echo htmlspecialchars($tour_location) ?></small>
                <h4>Details</h4>
